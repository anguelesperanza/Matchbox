...
/*
	...
	sampled depth target -- and since P7b the AO texture, and since P7c the two
	localized-probe arrays. See `DEFERRED_LIGHTING_SAMPLER_COUNT`'s own doc
	comment (render.odin) for the count (14, which is the least headroom
	anything in this package has ever had) and `test_mesh_frag_sampler_count_is_pinned_under_vulkan_floor`
...
test_deferred_lighting_sampler_count_is_pinned_under_vulkan_floor :: proc(t: ^testing.T) {
	testing.expect_value(t, DEFERRED_LIGHTING_SAMPLER_COUNT, 14)
