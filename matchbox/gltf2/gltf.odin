...
accessors_free :: proc(accessors: []Accessor) {
    // MATCHBOX PATCH: used to also free accessor.sparse.?.indices/.values,
    // back when those were slices (`[]Accessor_Sparse_Indices` /
    // `[]Accessor_Sparse_Values`) allocated by sparse_indices_parse /
    // sparse_values_parse. Both are a single struct now, per the glTF spec --
    // see Accessor_Sparse's own comment -- so there is nothing left to free
    // but the accessors slice itself.
    delete(accessors)
}
...
accessor_sparse_parse :: proc(object: json.Object) -> (res: Accessor_Sparse, err: Error) {
    ...
    for k, v in object {
        ...
        case "indices":
            // Required. A single object, not an array -- see Accessor_Sparse's
            // own MATCHBOX PATCH comment in types.odin.
            res.indices = sparse_indices_parse(v.(json.Object)) or_return
            indices_set = true

        case "values":
            // Required, same shape as "indices" above.
...
sparse_indices_parse :: proc(object: json.Object) -> (res: Accessor_Sparse_Indices, err: Error) {
    ...
    if !buffer_view_set {
        return res, GLTF_Error{type = .Missing_Required_Parameter, proc_name = #procedure, param = {name = "bufferView"}}
    }
    if !component_type_set {
        return res,
            GLTF_Error{type = .Missing_Required_Parameter, proc_name = #procedure, param = {name = "componentType"}}
    }
