/*
    ...
    how the read itself works, so `shadow_sample_pcf` (declared earlier in
    `lighting_core.hlsli`'s own include order) is reused rather than copied.
    ...
    **`Cascade_Data` is this technique's own uniform block**, not part of the
    ...
    `cascade_maps`/`cascade_samplers` are declared by whichever shader
...
/*
    ...
    cascade's own `shadow_sample_pcf` call fades it to fully lit once it
    falls outside that map's own frustum, the same out-of-range degrade
...
float shadow_visibility_cascaded(int light_index, float3 world, float3 normal)
{
    ...
    if (light_index == int(flags.z))
    {
        ...
        return shadow_sample_pcf(cascade_maps[i], cascade_samplers[i], cascade_view_projection[i], world, normal, bias);
    }
