...
void main()
{
    ...
    vec2 uv_final_ = vec2_ZERO;
    uv_final_ = uv_;
    if((data_._res_.flip_x_ != 0))
    {
        uv_final_.x = (1.0 - uv_final_.x);
    }

    if((data_._res_.flip_y_ != 0))
    {
        uv_final_.y = (1.0 - uv_final_.y);
    }

    _res_out_loc0_ = texture_sample(data_._res_.texture_a_, data_._res_.s_, uv_final_);
}
